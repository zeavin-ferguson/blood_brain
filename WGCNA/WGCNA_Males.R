...
A2Data <- read.delim(paste0(directory, "voom_amy_M.txt"))
dim(A2Data)
...
#sizeGrWindow(12, 9)
pdf(file = paste0(directory, "network_dendrogram_M_amy_merged_mods.pdf"), wi = 9, he = 6)
...
METree = hclust(as.dist(MEDiss), method = "average");
# # Plot the result
file_name<-paste0(directory,"/ME_dendrogram_M_amy_merged_mods.pdf")
...
#write.csv(p_matrix, paste0(directory,"CIE_M_amy_module_trait_P_table.csv"))
#write.csv(dataMatrix2, paste0(directory,"CIE_M_amy_module_trait_corr_table.csv"))


file_name<-paste0(directory,"ME_trait_correlations_amy_M_merged_MEs.pdf")
...
for ( i in 1:length(module)) {
  ...
  pdf(file=paste0(directory, module[[i]], "_corr_vs_MM_M_amy.pdf"))
  ...
  file_name=paste0(directory, module[[i]], "_genes_M_amy.csv")
...
file_name<-paste0(directory, "geneInfo_M_amy.csv")
...
cyt = exportNetworkToCytoscape(modTOM,edgeFile = paste("CytoscapeInput_edges_CIE_M_amy", paste(module, collapse="_"), ".txt", sep=""),nodeFile = paste("CytoscapeInput_nodes_CIE_M_amy", paste(module, collapse="_"), ".txt", sep=""),weighted = TRUE,threshold = 0.02,nodeNames = mod_ensemblIDs, altNodeNames = modGenes, nodeAttr = dynamicColors[inModule])
